...
export function printReport(report: ScanReport, options: ScanOptions = {}): void {
  if (options.json) {
  ...
  console.log(
    `  Cleanliness Score: ${scoreColor(c.bold(`${report.score}/100`))} ` +
  ...
  for (const [filePath, fileIssues] of fileMap.entries()) {
    ...
    for (const issue of fileIssues) {
      ...
      if (issue.severity === 'warning') tag = c.bgYellow(c.bold(' WARN  '));
      if (issue.severity === 'info') tag = c.bgBlue(c.bold(' INFO  '));
